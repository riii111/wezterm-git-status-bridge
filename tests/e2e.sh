...
run_update() {
	cache_path=$1
	cwd_path=$2
	"$bin" update --cache-dir "$cache_path" --pane-id "$pane_id" --cwd "$cwd_path"
	test -f "$cache_path/herdr-git-info-by-pane/$sanitized_pane_id"
}
...
run_update "$clean_cache" "$clean_repo"
run_update "$dirty_cache" "$dirty_repo"
run_update "$non_repo_cache" "$non_repo"
